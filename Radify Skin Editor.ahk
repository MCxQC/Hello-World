...
#Include <v2\object_list\object_list_mod>
 ...
 * @license MIT
 * @date 2025-07
 * @see {@link https://github.com/XMCQCX/Radify GitHub}
...
class RadifySkinEditor {
    static __New()
    {
        ...
        this.linkGitHubRepo := 'https://github.com/XMCQCX/Radify'
    ...
    static gSkin_Show(*)
    {
        ...
        m := this.gMain.Submit(false)

        for settings in this.arrSettings {
            switch settings[2], false {
                case 'textFontOptions': user := this.BuildFontOptionsString()
                default: user := Radify.range.HasOwnProp(settings[2]) ? Radify.ClampValue(m.%settings[2]%, settings[2]) : m.%settings[2]%
            }
            if (skin = 'default') {
                col3 := Radify.CapDecimals(Radify.defaults.%settings[2]%)
                col4 := Radify.originalDefaults.%settings[2]%
            } else {
                col3 := this.HasVal(settings[2], Radify.skins.%skin%.arrKeyDefined) ? Radify.skins.%skin%.%settings[2]% : ''
        ...
        for settings in this.arrSettings {
            ...
            if (skin = 'default') {
                ...
                if (this.HasVal(key, Radify.skins.%skin%.arrKeyDefined)) {
    ...
    static gSkin_btnOK_btnApply_Click(skin, ctrlObj, *)
    {
        ...
        if (skin = 'default') {
            ...
            skinObj.arrKeyDefined := Array()
            this.skinCurrent := {}

            for key, value in this.skinTemp.OwnProps() {
                skinObj.%key% := value
                skinObj.arrKeyDefined.Push(key)
    ...
    static SaveToJSONpreferences()
    {
        ...
        for skin, skinObj in Radify.skins.OwnProps() {
            switch skin, false {
                ...
                default:
                    m.skins.%skin% := {}
                    for k in skinObj.arrKeyDefined
        ...
        if (skin = 'default') {
            for key, skinObj in Radify.skins.OwnProps() {
                for k, v in skinObj.Clone().OwnProps()
                    if (!this.HasVal(k, skinObj.arrKeyDefined) && k != 'arrKeyDefined')
    ...
    static gSkin_Check_Basic_Skin(skin, param, *)
    {
        ...
        for (settings in this.arrSettings) {
            ...
            if (this.HasVal(key, param = 'skin' ? Radify.skins.%skin%.arrKeyDefined : this.arrSettingsBasic))
                this.skinTemp.%key% := rowTxt

            this.gSkin.lv.Modify(A_Index, this.HasVal(key, param = 'skin' ? Radify.skins.%skin%.arrKeyDefined : this.arrSettingsBasic) ? 'Check' : '-Check')
        }
    ...
    static gDir_Show(*)
    {
        ...
        (
            ...
            The rootDir placeholder represents the directory containing the Radify library file (Radify.ahk).
        )')
